    ...
    <div class="mt-7 bg-red-800 p-3 rounded">
        <div class="flex align-item justify-between">
            <div class="flex gap-3">
                <img
                    :src="post.user_id.avatar_url || 'https://i.pinimg.com/736x/8b/16/7a/8b167af653c2399dd93b952a48740620.jpg'"
                ...
                <div>
                    <div class="text-[15px] font-bold pb-1 flex items-center ">
                        <p class="hover:underline cursor-pointer">
                            <Link :href="route('profile',post.user_id.username)">
                                {{post.user_id.name}}
                            </Link>
                        </p>
                        <template v-if="post.group_name">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                 stroke="currentColor" class="w-4 h-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/>
                            </svg>

                            <Link :href="'/g/' + post.group_slug" class="text-green-500 hover:underline cursor-pointer">{{post.group_name}}</Link>
                        </template>
                    </div>
                ...
                <div v-if="post.attachments.length === 1">
                    <div>
                        <div v-for="attachment in post.attachments" :key="attachment.id"
                            ...
                            <a :href="attachment.path" :data-lightbox="'post-' + post.id"
                               class="flex items-center justify-center">
                                <img v-if="isImage(attachment)" :src="attachment.path" alt="no-image"
                                     class="h-[300px] w-auto object-fill rounded">
                                <video v-else>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div v-for="attachment in post.attachments" :key="attachment.id"
                            ...
                            <a :href="attachment.path" :data-lightbox="'post-' + post.id"
                               class="flex items-center justify-center">
                                <img v-if="isImage(attachment)" :src="attachment.path" alt="no-image"
                                     class="h-[300px] w-auto object-cover rounded">
                                <video v-else>
                                    <source :src="attachment.path" type="video/mp4">
                                </video>
                            </a>
        ...
        <div class="grid grid-cols-3 mt-2 border-t-2 border-b-2 pt-1 pb-1 border-gray-300">
            <button class="flex items-center justify-center gap-2 w-full hover:bg-[#111827] p-1 rounded"
                ...
                <div class="flex gap-1">
                    ...
                    <span v-if="post.reaction_count > 0">{{ post.reaction_count }}</span>
                </div>
